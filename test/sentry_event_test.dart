...
void main() {
  group('deserialize', () {
    ...
    test('fromJson', () {
      ...
      expect(DeepCollectionEquality().equals(sentryEventJson, json), true);
    });
  ...
  group(SentryEvent, () {
    ...
    test('serializes to JSON', () {
      ...
      expect(
        SentryEvent(
          ...
          tags: const <String, String>{'a': 'b', 'c': 'd'},
          // ignore: deprecated_member_use_from_same_package
          extra: const <String, dynamic>{'e': 'f', 'g': 2},
    ...
    test('should serialize $SentryThread when no $SentryException present', () {
      final serialized = SentryEvent(
        threads: [
          SentryThread(id: 0, crashed: true, current: true, name: 'Isolate'),
        ],
      ).toJson();
    ...
    test('should serialize $SentryThread when id matches exception id', () {
      final serialized = SentryEvent(
        exceptions: [SentryException(type: 'foo', value: 'bar', threadId: 0)],
        threads: [
          SentryThread(id: 0, crashed: true, current: true, name: 'Isolate'),
        ],
      ).toJson();
    ...
    test('should not serialize event.threads.stacktrace '
        'if event.exception is set', () {
      // https://develop.sentry.dev/sdk/event-payloads/stacktrace/
      final stacktrace = SentryStackTrace(
        frames: [SentryStackFrame(function: 'main')],
      );
      final serialized = SentryEvent(
        exceptions: [
          SentryException(
    ...
    test('should serialize event.threads.stacktrace '
    ...
    test('serializes to JSON with sentryException', () {
      ...
      expect(serialized['exception']['values'].first['mechanism'], {
        'type': 'mech-type',
        'description': 'a description',
